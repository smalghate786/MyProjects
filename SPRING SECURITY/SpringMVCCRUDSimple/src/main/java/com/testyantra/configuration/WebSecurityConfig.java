...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	/*	http.authorizeRequests().antMatchers("/empform", "/viewemp").hasAnyAuthority("admin")
		.antMatchers("/deleteemp", "/editsave", "/editemp").hasAuthority("admin")

		.antMatchers("/login/*").permitAll().anyRequest().authenticated().and().formLogin()
		.defaultSuccessUrl("/success").and().logout()
		.logoutRequestMatcher(new AntPathRequestMatcher("/logout")).and().exceptionHandling()
		.accessDeniedPage("/logout");*/
		
		http.authorizeRequests().antMatchers("/empform", "/viewemp").hasAnyAuthority("admin")
		.antMatchers("/deleteemp", "/editsave", "/editemp").hasAuthority("admin")
		.antMatchers("/empform").permitAll()
